import type { Middleware } from '../typing'
// 参考自 puck-core 请求库的插件机制
import compose from './compose';
class Onion<T> {
  constructor(defaultMiddlewares: Middleware<T>[]) {
  ...
  defaultMiddlewares: Middleware<T>[]

  middlewares: Middleware<T>[]

  static globalMiddlewares: Middleware<any>[] = []; // 全局中间件
  static defaultGlobalMiddlewaresLength = 0; // 内置全局中间件长度
  static coreMiddlewares: Middleware<any>[] = []; // 内核中间件
  static defaultCoreMiddlewaresLength = 0; // 内置内核中间件长度

  use(newMiddleware: Middleware<T>, opts: Partial<{
    global: boolean;
    core: boolean;
    defaultInstance: boolean;
  }> = { global: false, core: false, defaultInstance: false }) {
    let core = false;
    let global = false;
